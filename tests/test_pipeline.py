...
class Pipeline(unittest.IsolatedAsyncioTestCase):
        ...
        for side in (Side.LONG, Side.SHORT):
            with self.subTest(side=side):
                ...
                settings = replace(Settings.from_env(), symbols=('BTC_USDT',),
                    data_mode='live', high_leverage_lab=True, startup_warmup_seconds=0, regime_confirm_seconds=0)
                ...
                # The restored strategy requires ten ready assets. Only BTC has liquidations.
                features = {'BTC_USDT': feature}
                for i in range(9):
                    symbol = f'TEST{i}_USDT'
                    market.symbol(symbol)
                    features[symbol] = replace(feature, symbol=symbol, liquidation_events_300s=0,
                                               liquidation_notional_300s=0)
                signals = router.evaluate_all(market.symbols, features, now)
    ...
    async def test_rejection_explains_impossible_leverage(self):
        settings = replace(Settings.from_env(), symbols=('BTC_USDT',), data_mode='live', high_leverage_lab=True)
        market = MarketState(settings.symbols)
        state = market.symbol('BTC_USDT')
