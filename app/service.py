...
class PaperTradingService:
    ...
    async def start(self) -> None:
        ...
        LOGGER.info("RELEASE v0.5.1 accounts=%s execution=PAPER_ONLY venues=MEXC_EXECUTION,BYBIT_LIQUIDATION,BINANCE_CONFIRMATION reversal_regimes=RANGE,STRESS flow_proxy_executable=false",
    ...
    async def _refresh_mexc(self, mexc) -> None:
        while not self._stopping:
            ...
            try:
                ...
                LOGGER.exception('MEXC periodic refresh failed; retrying next cycle')
    ...
    async def _engine_loop(self) -> None:
        while not self._stopping:
            ...
            try:
                ...
                for symbol in features.keys() - signal_features.keys():
                    self.router.composite.last_diagnostics[symbol] = {
                        'symbol': symbol, 'ts': tick_started, 'state': 'universe_ineligible',
                        'data_ready': features[symbol].data_ready, 'blockers': ['universe_ineligible']}
                tick_decisions = []
                ...
                await self.storage.maintain(tick_started)
                if tick_started - self._last_feature_persist >= max(60, self.settings.feature_persist_seconds):
    ...
    async def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "storage_free_bytes": self.storage.free_bytes,
            "telemetry_enabled": self.storage.telemetry_enabled,
