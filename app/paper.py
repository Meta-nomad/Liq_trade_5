...
class PaperBroker:
    ...
    def open_from_signal(self, account: AccountState, signal: Signal, now: float) -> Position | None:
        ...
        target_move_pct = (signal.target_r * actual_risk / notional + 2*self.settings.taker_fee_rate) / (1 - float(signal.side)*self.settings.taker_fee_rate)
        if self.settings.high_leverage_lab:
        ...
        logging.getLogger(__name__).info("PAPER POSITION account=%s leverage=%.0f margin=%.4f notional=%.4f risk=%.4f target_net_r=%.2f liquidation_model=APPROXIMATE", account.name, position.leverage, position.margin, position.notional, position.initial_risk_usdt, position.target_r)
    ...
    def close_position(
        ...
        *,
        ...
        account.cooldowns[position.symbol] = now + self.settings.cooldown_seconds
        self._roll_risk_periods(account, now)
        return trade
    ...
    def evaluate_positions(
        ...
        for account in self.accounts.values():
            ...
            for symbol, position in list(account.positions.items()):
                ...
                if not self.settings.high_leverage_lab:
                    exit_fill = self._fill(symbol, Side(-int(position.side)), position.qty*mark, now=current_time, base_qty=position.qty)
                    expected_net = (position.unrealized_pnl(exit_fill.price) - position.entry_fee - position.qty*exit_fill.price*self.settings.taker_fee_rate + position.accrued_funding) if exit_fill else float('-inf')
                    target_hit = expected_net >= position.target_r*position.initial_risk_usdt
                if self.settings.high_leverage_lab:
                    exit_fill = self._fill(symbol, Side(-int(position.side)), position.qty*mark, now=current_time, base_qty=position.qty)
