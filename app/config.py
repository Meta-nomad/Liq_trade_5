...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        symbols_raw = os.getenv("FLOW_SYMBOLS", ",".join(DEFAULT_SYMBOLS[:30]))
        symbols = tuple(
        ...
        # A separate ledger prevents mixing incompatible experiments.
        db_path = Path(os.getenv("FLOW_DB_PATH", str(Path(os.getenv("RAILWAY_VOLUME_MOUNT_PATH", "data")) / "flow_v050.db"))).expanduser()
        return cls(
            high_leverage_lab=False,
            ...
            reversal_interval_seconds=_float("FLOW_REVERSAL_INTERVAL_SECONDS", 60.0),
            symbols=symbols,
            data_mode=data_mode,
            db_path=db_path,
            ...
            composite_low_risk_pct=_float("FLOW_LOW_RISK_PCT", 0.35),
            composite_base_risk_pct=_float("FLOW_BASE_RISK_PCT", 0.50),
            composite_high_risk_pct=_float("FLOW_HIGH_RISK_PCT", 0.60),
            base_max_leverage=_float("FLOW_MAX_LEVERAGE", 5.0),
            turbo_max_leverage=_float("TURBO_MAX_LEVERAGE", 8.0),
            max_margin_utilization=_float("MAX_MARGIN_UTILIZATION", 0.75),
            max_portfolio_risk_pct=_float("FLOW_PORTFOLIO_RISK_PCT", 1.20),
            max_open_positions=_int("FLOW_MAX_OPEN_POSITIONS", 3),
            ...
            signal_threshold=_float("FLOW_SIGNAL_THRESHOLD", 86.0),
            ...
            min_ready_ratio=_float("FLOW_MIN_READY_RATIO", 0.50),
            max_entry_spread_bps=_float("FLOW_MAX_SPREAD_BPS", 10.0),
            ...
            cooldown_seconds=_int("FLOW_COOLDOWN_SECONDS", 900),
